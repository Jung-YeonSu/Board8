...
public class PdsFile {
	...
	public static void save(
		...
		// map 객체 숫자로
		//int uploadPath = Integer.parseInt(String.valueOf(map.get("uploadPath")));
		
		// 저장될 경로를 가져온다.
		// application.properties의 part4.upload-path값
		// d:\\dev\\data\\
		String uploadPath = String.valueOf(map.get("uploadPath"));
		
		// 파일들을 저장하고 Files table 에 저장할 정보를 map에 담는다.
		...
		for (MultipartFile uploadfile : uploadfiles) {
			...
			// 파일 중복방지 : 같은 폴더에 같은 파일명의 파일을 저장하면
			//                 마지막 파일만 남는다.
			...
			// 저장된 파일들의 정보를 map 에  List방식으로 저장 
			// -> pdsServiceImpl 에 전달
	...
	private static String makeFolder(String uploadPath) {
		...
		if(uploadPathFolder.exists() == false) {
			...
			// mkdir()  : 상위폴더 없으면 폴대 생성 X
			// mkdirs() : 상위폴더 없어도 폴더전체를 생성 
		}
		return folderPath;	
	}
